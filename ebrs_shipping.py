...
for site_id in cluster['site']:
    ...
    while (count < 3):
        ...
        if subprocess.call(['ping', param, '1', site['ip_address']]) == 0:
            ...
            push_edrs = "rsync " + "-r $WORKSPACE/ebrs_facility " + site['username'] + "@" + site['ip_address'] + ":/var/www/"
            os.system(push_edrs)

            # Pushing edrs_setup script
            push_edrs_setup_script = "rsync " + "-r $WORKSPACE/ebrs_setup.sh " + site['username'] + "@" + site['ip_address'] + ":/var/www/ebrs_facility"
            os.system(push_edrs_setup_script)

            # setting up edrs application
            run_edrs_setup_script = "ssh " + site['username'] + "@" + site['ip_address'] + " 'cd / && ./ebrs_setup.sh'"
            ...
            for recipient in recipients:
                msg = "Hi there,\n\nDeployment of eBRS Facility for " + site['name'] + " completed succesfully.\n\nThanks!\nEGPAF HIS."
                params = {
            ...
            if count == 3:
                for recipient in recipients:
                    ...
                    msg = "Hi there,\n\nDeployment of eBRS Facility for " + site['name'] + " failed to complete after several connection attempts.\n\nThanks!\nEGPAF HIS."
                    params = {
                        "api_key": os.getenv('API_KEY'),
                        "recipient": recipient,
                        "message": msg
